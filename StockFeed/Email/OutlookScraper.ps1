...
foreach ($email in $inbox) {
  if ($email.ReceivedByName -eq "stock") {
    if ($email.attachments.count -ge 1) {
      $realshit = $email.receivedTime.GetDateTimeFormats()[5]
      echo ($realshit)
      if (($realshit) -eq ($Today)) {
        foreach ($attachment in $email.attachments) {
          if ($attachment.filename.contains(".CSV") -or $attachment.filename.contains(".csv") -or $attachment.filename.contains(".xlsx") -or $attachment.filename.contains(".zip")) {
